function! dapper#dap#Terminaterguments#new() abort
  let l:new = {
    \ 'TYPE': {'TerminateArguments': 1},
    \ 'restart': v:false,
  \ }
  return l:new
endfunction

function! dapper#dap#TerminateArguments#CheckType(object) abort
  if type(a:object) !=# v:t_dict || !has_key(a:object, 'TYPE') || !has_key(a:object['TYPE'], 'TerminateArguments')
  try
    let l:err = '(dapper#dap#TerminateArguments) Object is not of type TerminateArguments: '.string(a:object)
  catch
    redir => l:object
    silent! echo a:object
    redir end
    let l:err = '(dapper#dap#TerminateArguments) This object failed type check: '.l:object
  endtry
  throw l:err
  endif
endfunction
